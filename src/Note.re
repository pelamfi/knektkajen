...
let asNoteNameClass = (n: noteName): noteNameClass => {
    switch(n) {
     | DSharpEFlat(sharpOrFlatValue) | CSharpDFlat(sharpOrFlatValue)
     | FSharpGFlat(sharpOrFlatValue) | GSharpAFlat(sharpOrFlatValue)
...
let asNoteName = (n: chromaticNote, c: noteNameClass): noteName => {
    let sharpOrFlatValue: sharpOrFlat = switch(c) {
        | Flat => Flat
        | Natural => Sharp
        | Sharp => Sharp
    }
