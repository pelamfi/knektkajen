...
let string_of_state = (state: state): string => {
    "[current:" ++ string_of_int(state.current) 
    ++ " slideState: " ++ string_of_slide_state(state.slideState) 
    ++ " animationToIndexQueued: " ++ (state.animationToIndexQueued |> mapWithDefault(_, "none", string_of_int))
    ++ "]"
}
...
let string_of_event = (event: event): string => {
    switch(event) {
        ...
        | ChangeCurrent(current) => "ChangeCurrent(" ++ string_of_int(current) ++ ")"
    }
}
...
let paddingCount = (slideState: slideState, maxJump: int): int => {
    switch(slideState) {
        | Idle => 1 + maxJump
        | Animating({fromIndex, toIndex}) => min(max(1, (1 + maxJump) - (toIndex - fromIndex)), 1 + 2 * maxJump)
    }
}
...
let make = (~config: config, ~current: int) => {
  ...
  let (state, dispatch) =
            ...
            let newState = switch(action) {
                ...
                switch (state.slideState) {
                    ...
                    switch state.animationToIndexQueued {
                        |  Some(queued) =>
                            {...state, slideState: Animating({fromIndex: toIndex, toIndex: queued})}
                        | None =>
                ...
                | ChangeCurrent(newCurrent) =>
                    switch (state.slideState) {
                        | Idle => {...state, current: newCurrent, slideState: Animating({fromIndex: state.current, toIndex: newCurrent})}
                        | Animating(_) =>
                            {...state, current: newCurrent, animationToIndexQueued: Some(newCurrent)}
                    }
            };
    ...
    let paddingAnimClass = switch(state.slideState) {
        | Animating(_) => " " ++ config.styleBaseName ++ "Padding-anim"
        | Idle => ""
    }

    let paddingClass = config.styleBaseName ++ "Padding-" ++ string_of_int(paddingCount(state.slideState, config.maxJump))
        ++ paddingAnimClass;
